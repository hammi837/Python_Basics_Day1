...
def delete_expense(expense_id):
    ...
    new_expenses = [line for line in expenses if line.strip().split(',')[0] != str(expense_id)]
...
def update_expense(expense_id, amount=None, category=None, item=None):
    updated = False
    with open('expenses.csv', 'r') as f:
        expenses = f.readlines()
    ...
    for i, line in enumerate(expenses):
        ...
        if parts[0] == str(expense_id):  # match the ID in CSV
